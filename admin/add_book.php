...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = mysqli_prepare($conn,
            "INSERT INTO books (title, author, isbn, category, quantity, available, description)
             VALUES (?, ?, ?, ?, ?, ?, ?)"
        );
        mysqli_stmt_bind_param($stmt, 'ssssiis', $title, $author, $isbn, $category, $quantity, $quantity, $description);

        if (mysqli_stmt_execute($stmt)) {
            flashMessage('success', "Book \"$title\" added successfully.");
            redirect('books.php');
        } else {
...
<head>
    ...
    <title>Add Book – BookMyLibrary</title>
            ...
            <div class="col-lg-8">
                <div class="card card-shadow">
                    <div class="card-header-custom">
                        <h6 class="mb-0"><i class="fas fa-plus-circle me-2 text-primary"></i>New Book Details</h6>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($errors)): ?>
